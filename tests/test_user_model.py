...
class UserModelTestCase(unittest.TestCase):
    ...
    def test_elevate_user(self):
        ...
        db.session.add_all([a, u, m, user])
        db.session.commit()
        a.elevate(user=user, role=Role.query.filter_by(name='Moderator').first())
        self.assertTrue(user.role == m.role)
        a.elevate(user=user, role=Role.query.filter_by(name='Administrator').first())
        self.assertTrue(user.role == a.role)
        self.assertRaises(PermissionError, m.elevate, a)
        m.elevate(u)
        self.assertTrue(u.role == Role.query.filter_by(name='User').first())
        self.assertRaises(PermissionError, u.elevate, User(name='2'))
        self.assertRaises(PermissionError, u.elevate, u)
    ...
    def test_remove_from_shop(self):
        a = User(name='sorvihead', password='123', role=Role.query.filter_by(name='Administrator').first())
        m = User(name='sorvihead', password='123', role=Role.query.filter_by(name='Moderator').first())
        user = User(name='sorvihead', password='123', role=Role.query.filter_by(name='User').first())
        u = User(name='test', password='1')
        ...
        self.assertFalse(user.shop == s)
        a.add_to_shop(user, s)
        self.assertRaises(PermissionError, u.remove_from_shop, user, s)
